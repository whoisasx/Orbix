...
export const Bitcoin = () => {
	...
	const handleAddWallet = () => {
		...
		try {
			...
			showToast.success(
				`Bitcoin wallet ${len + 1} created successfully! ₿`
			);
		} catch (error) {
			...
			{bitcoins.length === 0 ? (
				...
				>
					<div className="w-20 h-20 mx-auto bg-linear-to-br from-orange-500 to-yellow-500 rounded-2xl flex items-center justify-center mb-6">
						<FaBitcoin className="w-10 h-10 text-white" />
					</div>
						...
						<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
							...
							<div className="flex items-center gap-3 w-full sm:w-auto">
								<motion.button
									...
									whileHover={{ scale: 1.02 }}
									whileTap={{ scale: 0.98 }}
									className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2 bg-linear-to-r from-orange-500 to-yellow-500 text-white font-medium rounded-xl hover:from-orange-600 hover:to-yellow-600 transition-all duration-200 shadow-md hover:shadow-lg"
								>
						...
						>
							...
							>
								...
								<p className="text-gray-700 dark:text-gray-300 mb-6">
									...
									{bitcoins.length} Bitcoin wallets? Make sure
